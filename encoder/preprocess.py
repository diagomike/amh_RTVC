...
class DatasetLog:
    ...
    def __init__(self, root, name):
        self.text_file = open(Path(root, "Log_%s.txt" % name.replace("/", "_")), "w",encoding='utf8')
...
def _preprocess_speaker(speaker_dir: Path, datasets_root: Path, out_dir: Path, skip_existing: bool):
    ...
    if sources_fpath.exists():
        try:
            with sources_fpath.open("r",encoding='utf8') as sources_file:
    ...
    sources_file = sources_fpath.open("a" if skip_existing else "w",encoding='utf8')
    audio_durs = []
...
def preprocess_voxceleb1(datasets_root: Path, out_dir: Path, skip_existing=False):
    ...
    with dataset_root.joinpath("vox1_meta.csv").open("r",encoding='utf8') as metafile:
